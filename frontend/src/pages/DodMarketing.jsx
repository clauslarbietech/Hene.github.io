...
export default function DodMarketing({ site }) {
  const data = site.dodMarketing;
  const [lightbox, setLightbox] = useState(null);

  const open = (images, index, title) => {
    setLightbox({ images: images.map(asset), index, title });
  };
  ...
  return (
    <section className="page project-page">
      <header className="page__header">
        ...
        <p className="project-disclaimer">{data.disclaimer}</p>
          ...
          <div className="reel">
            {section.images.map((src, i) => (
              <button
                ...
                onClick={() => open(section.images, i, section.title)}
